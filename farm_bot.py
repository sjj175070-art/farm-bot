...
async def btn(update, ctx):
    ...
    if cb == "upgrade_well":
        if u["well"] >= 5:
            await q.answer("Максимальный уровень!", show_alert=True)
            return
        next_lvl = WELL_LEVELS[u["well"] + 1]
        if u["usd"] < next_lvl["upgrade_price"]:
            await q.answer("Нужно $" + str(next_lvl["upgrade_price"]), show_alert=True)
            return
        if u["water"] < next_lvl["upgrade_water"]:
            await q.answer("Нужно " + str(next_lvl["upgrade_water"]) + " воды", show_alert=True)
            return
    ...
    if cb == "harvest":
        if not u["growing"]:
            await q.answer("Нечего собирать!", show_alert=True)
            return
        left = int(u["growing"]["finish"] - time.time())
        if left > 0:
            await q.answer("Ещё " + str(left // 60) + " мин!", show_alert=True)
            return
        veg = VEGS[u["growing"]["veg"]]
        amt = u["growing"]["amount"]
        harvest = veg["harvest"] * amt
    ...
    if cb.startswith("sow_"):
        ...
        water_need = amount * 5
        if u["usd"] < seed_cost:
            await q.answer("Нужно $" + str(seed_cost) + " на семена!", show_alert=True)
            return
        if u["water"] < water_need:
            await q.answer("Нужно " + str(water_need) + " воды!", show_alert=True)
            return
        if u["growing"]:
            await q.answer("Уже что-то растёт!", show_alert=True)
            return
    ...
    if cb == "make_fertilizer":
        if not u["factory"]:
            await q.answer("Нет завода!", show_alert=True)
            return
        if u["grass"] < 200:
            await q.answer("Нужно 200 травы!", show_alert=True)
            return
        batches = u["grass"] // 200
        u["grass"] -= batches * 200
    ...
    if cb == "sell_all":
        ...
        for vid, cnt in u["harvest"].items():
            if cnt > 0:
                ...
                has = True
        if not has:
            await q.answer("Нечего продавать!", show_alert=True)
            return
        u["harvest"] = {}
        u["usd"] += total
        save_user(uid, u)
        ...
        if not offers:
            ...
            for i, o in enumerate(offers[:5]):
                ...
                if veg:
                    ...
                    rows.append([InlineKeyboardButton("Купить " + veg["name"] + " x" + str(of["amount"]),
                                                      callback_data="buy_offer_" + o["seller"] + "_" + str(of["id"]))])
